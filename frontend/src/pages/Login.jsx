  ...
  try {
    ...
    if(state === 'sign Up'){
      ...
      if(data.success){
       ...
       const {data} = await axios.post(backendUrl + '/api/user/login', { email, password}) //already have state variable for name, email, password linked with input fields
...
useEffect(()=>{
  if(token){
    navigate('/') //if token gets updated  means we have just register or logged in so navigate to home page
  ...
  return (
    <form onSubmit={onSubmitHandler} className='min-h-[80vh] flex items-center'>
      <div className = 'flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg'>
        ...
        <button type="submit" className ='bg-primary text-white w-full py-2 rounded-md tex-base'>{state === 'Sign Up' ? "Create Account" : "Login"}</button>
        {
